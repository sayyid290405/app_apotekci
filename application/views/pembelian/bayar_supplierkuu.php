            ...
            <form method="get" class="row g-3 align-items-center">
                ...
                <div class="col-auto">
                    <select name="filter" class="form-select" onchange="this.form.submit()">
                        ...
                        <option value="menunggu" <?= $this->input->get('filter') == 'menunggu' ? 'selected' : '' ?>>Menunggu Pembayaran</option>
                        <option value="disetujui" <?= $this->input->get('filter') == 'disetujui' ? 'selected' : '' ?>>Sudah Upload (Menunggu Approve)</option>
                                ...
                                <td>
                                    ...
                                    <?php elseif($p->status == 'disetujui' && $p->id_transaksi != NULL): ?>
                                        <form action="<?= base_url('Pembelian/approve/'.$p->id_pembelian) ?>" 
                                              method="post"
                                              onsubmit="return confirm('⚠️ Approve pembelian ini? Stok akan otomatis bertambah.')">
                                            <input type="hidden" 
                                                   name="<?= $this->security->get_csrf_token_name(); ?>" 
                                                   value="<?= $this->security->get_csrf_hash(); ?>">
                                            <button type="submit" class="btn btn-success btn-sm w-100">
                                                <i class="fas fa-check-circle"></i> Approve & Tambah Stok
                                            </button>
                                        </form>
